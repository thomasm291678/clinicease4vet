...
def update_record(record_id):
    ...
    try:
        cursor.execute(
            """UPDATE medical_records SET vet_name=?, visit_date=?, diagnosis=?,
               treatment=?, symptoms=?, subjective=?, objective=?,
               assessment=?, plan=?, notes=?, follow_up_date=?, fee_charged=? WHERE id=?""",
            (
                data.get("vet_name", ""),
                data.get("visit_date", ""),
                data.get("diagnosis", ""),
