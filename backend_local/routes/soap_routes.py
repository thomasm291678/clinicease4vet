...
def get_soap(record_id):
    ...
    # 获取临床推理
    cursor.execute("SELECT * FROM clinical_reasoning WHERE record_id = ?", (record_id,))
...
def update_soap(record_id):
    ...
    # 更新 SOAP 字段
    cursor.execute(
        """UPDATE medical_records SET subjective=?, objective=?,
           assessment=?, plan=? WHERE id=?""",
        (
            data.get("subjective", ""),
            data.get("objective", ""),
            data.get("assessment", ""),
    ...
    if reasoning:
        cursor.execute("SELECT id FROM clinical_reasoning WHERE record_id = ?", (record_id,))
        ...
        if existing:
            cursor.execute(
                """UPDATE clinical_reasoning SET problem_list=?, reasoning_path=?,
                   differential_list=?, must_not_miss=?, missing_info=?,
                   recommended_tests=?, dynamic_questions=?,
                   client_communication=?, summary=? WHERE record_id=?""",
                (
...
def regenerate_reasoning(record_id):
    ...
    try:
        ...
        cursor.execute(
            "SELECT diagnosis, symptoms, subjective, objective, assessment, plan FROM medical_records WHERE id = ?",
            (record_id,),
        )
        record = cursor.fetchone()
        if not record:
...
def regenerate_client_comm(record_id):
    ...
    try:
        ...
        cursor.execute("SELECT diagnosis, plan FROM medical_records WHERE id = ?", (record_id,))
        record = cursor.fetchone()
        if not record:
            return jsonify({"error": "记录不存在"}), 404
        ...
        if existing:
            cursor.execute(
                "UPDATE clinical_reasoning SET client_communication=? WHERE record_id=?",
                (client_json, record_id),
            )
        else:
            cursor.execute(
                "INSERT INTO clinical_reasoning (record_id, client_communication) VALUES (?, ?)",
